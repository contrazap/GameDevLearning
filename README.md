...
This repo holds the roadmap, plans, knowledge notes, and templates — **text only, no LFS**. The UE projects themselves live outside it, one folder + one GitHub repo per project (parent folder defined in [CONVENTIONS.md](CONVENTIONS.md): `/home/pallav/MyFiles/Projects/GameDevLearningPractice/`). During sessions this repo stays checked out beside the active project so plans and notes are updated in place.
...
7. **Linux is the dev host and the desktop ship target** (added 2026-07-23, superseding the original Windows assumption). The ladder runs on Arch Linux; UE 5.8 Linux editor, Blender via flatpak. Every project brief that said "packaged Windows build" now says **Linux** — Epic ships a Linux cross-compile toolchain for Windows *hosts*, not the reverse, so Windows binaries are out of reach from here. Android packaging is unaffected and stays in scope. What is being learned — the packaging step as part of finishing — is identical; only the target triple changes.
...
**Where the UE projects live:** `/home/pallav/MyFiles/Projects/GameDevLearningPractice/<ProjectFolder>/`. Each project is its own GitHub repo named after its folder (`A1_Flappy`, `A2_Clicker`, …). Morning sessions run in either the project folder or this repo; the other side is reached by absolute path.
