...
2. **Projects parent folder — single source of truth:** `/home/pallav/MyFiles/Projects/GameDevLearningPractice/` (created 2026-07-17 on Windows; ladder migrated to Linux 2026-07-23 — see S0 session log). Plans and notes reference project files as `A1_Flappy/Source/...` — always relative to this parent. The Windows path-length watch is retired (no `MAX_PATH` on ext4); the Linux replacement is **case sensitivity** — ext4 distinguishes `BP_Bird` from `bp_bird` where NTFS did not, so an asset reference or `#include` with wrong casing fails here and would have silently worked on Windows. Suspect casing first when a path "exists but isn't found".
...
4. **Editors.** On Linux there is no Visual Studio: **VS Code** (or Rider) for C++ *and* for plans, markdown, Claude Code. Create source files on disk → regenerate project files (from the editor's *Refresh VS Code Project*, or `GenerateProjectFiles.sh` / `UnrealBuildTool -projectfiles`) → build via UBT or the generated tasks. Never hand-edit generated project files. Red squiggles on engine includes are expected — **ignore them; the build is the source of truth.** Toolchain specifics land at A3.3 (first C++ project) and go on that plan's Verify list.
